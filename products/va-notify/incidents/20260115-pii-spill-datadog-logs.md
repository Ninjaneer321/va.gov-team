# Incident Report: VA Notify PII Spill to Datadog Logs

**Date:** 2026-01-15  
**Severity:** _[TBD]_  
**Status:** Resolved  
**Incident Commander:** _[TBD]_  
**Authors:** _[TBD]_

---

## Summary

_[TBD: summary from incident report]_

## Timeline

| Time (ET) | Event |
|-----------|-------|
| _[TBD]_ | Incident detected |
| _[TBD]_ | Mitigation applied |
| _[TBD]_ | Incident resolved |

## Impact

- **Duration:** _[TBD]_
- **Affected channels:** _[TBD]_ 
- **Estimated notifications affected:** _[TBD]_
- **Business lines affected:** _[TBD]_

## Root Cause

_[TBD: root cause from incident report]_

## Resolution

_[TBD: resolution details from incident report]_

## Action Items

| Action | Owner | Due Date | Status |
|--------|-------|----------|--------|
| _[TBD]_ | | | |

## Lessons Learned

_[TBD: lessons learned text from incident report]_
