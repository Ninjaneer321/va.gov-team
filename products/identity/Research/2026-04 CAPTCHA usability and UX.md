...
Audio CAPTCHAs are absolutely the most blocking to humans. They block almost half of human traffic that interacts with them. Unfortunately, this is also a fallback method given to blind users for every type of visual oriented CAPTCHA. It’s also the fallback for blind users who are flagged in some modern “invisible” types. It creates a substantial blocker. Image and puzzle based CAPTCHA don't have higher human success in general than text based, and actually blocks more human traffic than bot traffic, rendering these types mostly ineffective for the harm caused to users, contradicting their core design goal <sup>3, 7, 16</sup>  
Looking at the more modern types, ReCAPTCHA v3 is finally a product that allows over 90% of humans successfully through. But in cases of private browsing or assistive technology, it’ll flag and revert users through a secondary screening using ReCAPTCHA v2. That blocks over 20% of humans and a majority of folks with accessibility needs, many of whom are being sent through the secondary screening flow. Cloudflare Turnstile is the first CAPTCHA product that is WCAG 2.2 AAA compliant. It allows 99% of humans through and doesn't seem to cause significant hurdles for accessibility. This should be the only style considered as doing a decent job at allowing humans though.<sup>2, 9, 14</sup>
