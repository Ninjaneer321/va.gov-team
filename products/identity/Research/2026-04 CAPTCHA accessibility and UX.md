...
Cloudflare Turnstile, stands out as a WCAG 2.2 AAA compliant solution and allows almost 100% human success. As a tradeoff of that human ease, it does allow a lot of bot traffic through. This solution, or one that's substantially similar and also has a minimum of WCAG 2.2 AA compliance, are the only CAPTCHA solutions we should be considering for use on VA.gov. Also since these are relatively new, they warrent our own full accessibiltiy testing should we decide to move forward with one of these options.
