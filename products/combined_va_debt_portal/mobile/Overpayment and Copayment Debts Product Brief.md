...
* Total number of clicks on the links to access VA.gov. We expect that Veterans will want to request help or dispute their debt or copay, and that this non-zero amount will increase. We will track the total number of clicks on the links to access VA.gov on a monthly basis. 
* Total number of clicks on the "Make a Payment" buttons for Copays and Overpayments. These buttons will allow users to begin the payment journey for their overpayment debt (on pay.va.gov) or copayment bill (on pay.gov). We expect this non-zero amount will increase. We will track the total number of clicks on each button to access the respective payment sites on a monthly basis.
...
|3/9/2026| The copay tile will only be located on the Payment tab within the VA: Health and Benefits mobile app.| During our Design Review with Ryan he mentioned that the Copay tile should live on one of the screens. This feedback was considered launch blocking for MVP and Michelle signed off for our team to address.|
|3/9/2026| Feedback provided by Ryan regarding the VBA alerts will be addressed in v2.|During our Design Review, Ryan raised concerns that he had with the VBA alert content, as well as the action sheet/bottom sheet, including actions that might need to be links instead of buttons. These items will be noted in a separate epic and addressed post-MVP release.|
|3/25/2026| Copay MVP: The FM team will be providing the Oracle (old) experience to any facilities that are migrating from Vista to Oracle up to July 2027. TBD if Copay MVP is released onto the mobile app in the same manner. | The FM team will be providing the experience of Veterans that have Vista data with the new Healthcare Cost and Coverage API, which will allow for data to be more recent. The Veterans with Oracle/Cerner data will have the current API (Veteran Billing System or VBS), which means that data is not as recent and can be stake (generally data is updated around a month). Right now there is a push to have health systems migrate to Oracle/Cerner. This is referred to as the Federal EHR migration and you can see the deployment schedule [here](https://dvagov.sharepoint.com/sites/vafederalehrresourcecenter/SitePages/DeploymentSchedule.aspx). The FM team has complied a list of systems that are migrating from Vista to Oracle/Cerner up to July 2027. Those systems will not have the new API experience, so that they do not provide users with a better experience and then take it away. They will instead provide them with the the old experience until the Healthcare Cost and Coverage API includes Oracle/Cerner data (estimated to be around July 2027). Message [here](https://dsva.slack.com/archives/C07S7TJRK0B/p1774468051097069) |
|3/25/2026| Copay MVP: The FM team is planning to attend staging review on April 24th for the implementation of the new Healthcare Cost and Coverage API, which means they are targeting a release on VA.gov around May 2026.| Information was discussed during FM sync and at Team of Teams.|
|4/8/2026| Transaction history will not be included in Overpay MVP.| During our sync with the FM team, they let us know that the data for transaction history is being worked on and will not be part of MVP. MFS team will be working to put this behind a feature flag for release. |
