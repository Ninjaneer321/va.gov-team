...
- [Key findings for implementation](#key-findings-for-implementation)
  - [Key Finding 1: Wayfinding between pages](#key-finding-1-wayfinding-between-pages-is-inconsistent)
  - [Key Finding 2: Update list filter button](#key-finding-2-the-update-list-filter-button-is-easy-to-miss)
...
9 Veterans participated in moderated usability sessions evaluating new medication management pages (Medications Page, Medication History Page, In-Progress Medications Page). Overall, participants responded positively to the conceptual design, describing it as "intuitive" and "easy" (8 of 8 who completed the full session; P7 excluded due to a screen freeze during Task 3). However, two key usability issues emerged that may affect task completion in production:

1. **The filter "Update list" button is easy to miss.** 7 of 8 participants selected filters but didn't click "Update list" at least once during the session, believing the list had changed when it hadn't. Without clear feedback, participants continued interacting with the page as if their filter had been applied. (P7 excluded — moderator controlled the screen due to technical issues.)
2. **Wayfinding between pages is inconsistent.** 7 of 9 participants landed on the refill-only Medications Page as their first destination in Task 1, and cross-links designed to connect the three pages were missed by 3 participants.

**The central tension:** The new design separates medications across three distinct pages, but users don't arrive with a mental model of this three-page structure. They expect "Medications" or "Refill VA medications" to show everything. When users can't find what they need, their first instinct is often to turn to non-digital channels — 3 of 9 participants (P7, P12, P15) said they would call the pharmacy or send a secure message rather than continue navigating.
...
- 7 of 9 participants landed on the refill-only Medications Page as their first destination in Task 1. 5 clicked "Refill VA medications" from the MHV landing page (P1, P5, P7, P12, P15) and 2 used the Medications tab in the secondary nav (P4, P13). Only P8 and P16 started with "Review medications." When the medication wasn't there, recovery depended on noticing cross-links — which 3 participants missed.
...
**Cross-links are the primary mechanism, but discoverability is inconsistent.**
...
- When users missed cross-links, they either: went back to the MHV landing page (P5, P7, P12 in Task 1), used secondary nav or breadcrumbs (P4, P7, P13, P16), or said they'd call/message the pharmacy (P7, P12, P15).
...
### Key Finding 1: Wayfinding between pages is inconsistent

**Priority: Address before launch**

**The core observation:** The new design distributes medications across three pages, but users don't arrive with an understanding of this structure. They expect "Medications" or "Refill VA medications" to show them everything.

#### Part A: Users often start on a page that doesn't contain what they need
...
**Why this matters:** Users arrive expecting a single medications list. When they don't see what they're looking for, they may assume the medication isn't in the system rather than recognizing they need to navigate to a different page. 2 of 9 participants (P1, P7) explicitly interpreted the refill page as a full medication list.

#### Part B: Cross-links are the primary recovery mechanism, but they are frequently missed

The text-based links that connect the three pages were frequently overlooked or not understood. These links are doing important work in the design — they're how users are expected to move between pages when they start on one that doesn't have what they need.

| Link | Missed/not used by | Context |
|---|---|---|
...
#### Part C: When cross-links are missed, users take alternative paths or turn to non-digital channels

When users didn't notice the cross-links, they employed these strategies:
...
3. **Turning to non-digital channels.** 3 of 9 participants (P7, P12, P15) said they would call the pharmacy or send a secure message when they couldn't find information. P4 also said she would message her doctor about an old medication rather than use the filters to find it.
...
The cross-links work well when users see them. The challenge is visibility, not functionality. 3 of 9 participants (P4, P13, P1) noticed cross-links relatively quickly during Task 1 and navigated efficiently.
...
All 7 participants who submitted a refill in Task 3 (P1, P4, P5, P8, P12, P13, P16; P7 excluded due to screen freeze, P15 did not complete Task 3 as designed) noticed the green success alert, and 6 of 7 (P1, P5, P8, P12, P13, P16) clicked the "Go to your in-progress medications" link within it. This is a notably higher engagement rate than the static cross-links on the Medications Page. In Task 5, 4 of 6 participants (P5, P12, P13, P16) similarly used the success alert or the in-progress medications link to verify their bulk refill submissions.
...
|---|---|---|
| Recommended | Increase visual prominence of cross-links | Current text link treatment is not being seen; P12 specifically suggested making "the blue section" bigger |
| Recommended | Consider button-style treatment instead of text links | Buttons have higher visual weight and clearer affordance |
| Consider | Add contextual guidance when users land on a page that may not have what they need | E.g., if Medications Page shows 0 results, prompt user toward Medication History or In-Progress |
| Consider | Evaluate whether link labels clearly communicate destination | "In-progress medications" was not intuitive; "Review and print list of medications" was clearer |
...
### Key Finding 2: The "Update list" filter button is easy to miss

**Priority: Address before launch**

The filter "Update list" button was often missed. Participants selected filters but did not click "Update list," assuming the list had automatically updated. Without visible feedback that the filter had not been applied, participants continued interacting with the page — potentially reviewing or printing medication lists that did not reflect their intended selection.

7 of 8 participants missed the "Update list" button at least once when selecting a filter. By Task 4 (print), most had learned from earlier errors or moderator prompting — but 2 participants (P4, P16) still did not apply their filter before printing. (P7 excluded from all filter counts — moderator controlled the screen due to technical issues.)
...
**A notable example:** P4 selected "Renewal needed before refill" in Task 3 but never clicked "Update list." The list stayed on Active medications (the default), and Metformin happened to be visible because Active was the default view. P4 believed her filter had been applied correctly, so the mismatch went unnoticed. If the filter had been applied, Metformin would not have appeared in the "Renewal needed" list, masking a conceptual misunderstanding about refill vs. renewal.
...
**Recurring pattern:** P4 missed the button across multiple tasks (Tasks 3 and 4), suggesting this is not a learning curve issue but a mismatch with how users expect filters to behave. P16 also missed it across Tasks 2 and 4.

**Potential impact:**
- **Task 2 (find old medication):** 4 of 8 participants (P5, P8, P12, P15) who attempted to use the filter did not apply it correctly, leading some to believe the rash cream wasn't in the system because they thought they had filtered to Inactive medications when they had not. (P7 excluded — moderator controlled the screen.)
...
**Recommendation:** Consider auto-applying filters on radio button selection (this matches user expectation). If a manual apply button must remain, make it more prominent — e.g., a button that changes state when a new radio button is selected, or provide visible feedback that the list has not yet been updated.
...
**Discoverability:** The print feature is located at the bottom of the Medication History Page, below the medication list. 4 of 8 participants who attempted Task 4 had difficulty finding it (P7 excluded due to screen freeze). The print feature's placement below a potentially long medication list means Veterans must scroll through every entry before discovering it.
...
Of the 8 participants who attempted the task, 4 (P1, P8, P13, P16) found the print feature with relative ease, while 4 (P4, P5, P12, P15) had notable difficulty.

**Labeling:** P16 was confused by the "Submit" button within the print dropdown: "I assume that the submit was to submit the print request, but I don't know that an older veteran would understand that. They may be looking for a button that just says print." P4 viewed print options and selected "Download PDF" before clicking "Submit."
...
**Compounding concern:** When combined with the filter activation issue (Key Finding 2), there is a possibility of users printing a list that doesn't match their intent. In Task 4, 2 of 8 participants (P4, P16) selected filters but did not click "Update list," meaning any printout would not reflect their intended selection. (P7 excluded — moderator controlled the screen.)
...
The grey card's muted styling made it easy to miss on the In-Progress Medications Page. While all 9 participants who encountered the card correctly interpreted its meaning, 3 of 9 (P1, P12, P13) had difficulty initially locating it on the page, scrolling past it or not noticing it before it was pointed out.
...
- P12 missed the grey card entirely and needed moderator help to locate it. The muted visual treatment made it easy to overlook compared to the colored status sections.
...
Users understood the In-Progress page once they arrived, but the label "in-progress medications" did not clearly communicate what they would find there. 2 of 9 participants (P7, P8) explicitly expressed confusion about the term before selecting the link. The label did not clearly communicate what users would find there and requires users to already understand the concept it describes.

- P8: "Go to your in-progress medications. Now I'm confused, because I'm not sure what that one means."
- P7, when probed about the link: "I don't know what that is... I'm guessing that means you've placed the order."
- P8 also had difficulty distinguishing between "refill medications" and "in-progress medications": "Refill medication basically tells me the same thing..."

Once users saw the page, they reacted positively. 7 of 8 participants who viewed the page correctly interpreted the information shown (P7 excluded due to screen freeze; P5 did not visit the page during this task):
...
7 of 8 participants who viewed the page correctly interpreted the three-phase tracker (P7 excluded due to screen freeze). The 7 participants who interpreted the statuses were P1, P4, P8, P12, P13, P15, and P16. All 8 participants who saw the page — including P7 after the moderator navigated there — responded positively. Users valued the page and found it helpful.
...
**Default to non-digital channels when stuck:** When users encountered friction, their first instinct was often to turn to non-digital channels. 4 of 9 participants (P4, P7, P12, P15) expressed they would use non-digital or messaging channels as their primary fallback.
...
|---|---|---|
| Cross-links not prominent enough (missed by 3 participants in Task 1) | Increase visual prominence; consider button-style treatment | Key Finding 1 |
| Users often start on a page that doesn't have what they need (7 of 9 in Task 1) | Add contextual guidance when a page may not have what user needs; improve link labels | Key Finding 1 |
| "Update list" button is easy to miss (7 of 8 missed at least once; P7 excluded) | Auto-apply filters or make button more prominent | Key Finding 2 |
