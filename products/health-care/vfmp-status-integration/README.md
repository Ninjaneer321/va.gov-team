# VFMP Status Integration

## Team Information

### Basic Details

- **Team Name:** VFMP Status Integration
- **Team ID:** 33003
- **Short Name:** vfmp-status-integration
- **GitHub Team Name:** VFMP Status Tool
- **GitHub Team Labels:**
  - [vfmp-hst-sprint-planning]
- **GitHub Team Link:** https://github.com/department-of-veterans-affairs/va.gov-team/tree/master/products/health-care/vfmp-status-integration
- **Slack Channel:** [#vfmp-status-integration](https://dsva.slack.com/channels/vfmp-status-integration)

### Organizational Affiliation

- **VA Business Owner:** OCTO
- **Portfolio:** Health Portfolio
- **Crew or Pod:** VA Health Applications
- **Contracting Organization:** Agile Six
- **Contract:** 

### Team Roster

Directory system will parse this section to display team member contact information

#### Product Owner

- **Name:** Premal Shah
- **GitHub:** [@pshahVA](https://github.com/pshahVA)
- **Role:** PO

#### Design Lead

- **Name:** John Riggin
- **GitHub:** tbd
- **Role:** VA Design/Research Lead

#### Contractor Product Manager

- **Name:** Michael Mooney
- **GitHub:** [@ManOnTheMooney](https://github.com/ManOnTheMooney)
- **Role:** Product Manager

#### Engineering Lead

- **Name:** Kenny Santiago
- **GitHub:** [@ksantiagoBAH](https://github.com/ksantiagoBAH)
- **Role:** Engineering Lead

#### UX - Design/Research

- **Name:** Chantale Hedgeman
- **GitHub:** [clhedgeman](https://github.com/clhedgeman) 
- **Role:** UX

#### Engineer

- **Name:** Stephen Willis
- **GitHub:** tbd
- **Role:** Engineer

#### Engineer

- **Name:** tbd
- **GitHub:** tbd
- **Role:** Engineer

## About This Team

We will integrate CHAMPVA applications into the existing Claim and Appeal Status Tool (CST) to enhance transparency, reduce delays, and improve the overall user experience for veterans and their families. This solution aims to provide real-time access to application statuses, streamline documentation workflows, and reduce call center volume.
